...
printf '[3/12] tests\n'
go test ./... -count=1
printf '[4/12] race tests\n'
timeout --foreground 180s go test -race ./... -count=1
printf '[5/12] end-to-end\n'
...
printf '[9/12] checksums and archive safety\n'
...
printf '[10/12] binary smoke tests\n'
tmp=$(mktemp -d)
trap 'rm -rf "$tmp"' EXIT
...
printf '[11/12] packaged release E2E\n'
WORLDBISECT_E2E_TRACE=off ./scripts/release-e2e.sh \
  --archive "dist/worldbisect_$(cat VERSION)_linux_amd64.tar.gz" \
