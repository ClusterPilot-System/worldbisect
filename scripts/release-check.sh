...
printf '[3/10] tests\n'
go test ./... -count=1
printf '[4/10] race tests\n'
timeout --foreground 180s go test -race ./... -count=1
printf '[5/10] end-to-end\n'
timeout --foreground 120s ./scripts/e2e.sh
printf '[6/10] static contracts and web assets\n'
...
printf '[10/10] binary smoke tests\n'
tmp=$(mktemp -d)
trap 'rm -rf "$tmp"' EXIT
